...
const App: React.FC = () => {
  ...
  const constructPrompt = useCallback(() => {
    ...
    const compositionMap: Record<string, string> = {
      'front': 'facing forward, frontal view',
      'side': 'side profile view, facing left or right',
      'back': 'rear view, back facing camera',
      'three-quarter': 'three-quarter angle view',
      'dynamic': 'dynamic angle with movement',
    };
    ...
    const prompt = `
      ...
      Composition: ${compositionMap[config.composition]}.
        ...
        <div className="grid grid-cols-1 gap-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <select 
                    ...
                    className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-indigo-500 focus:outline-none appearance-none text-sm"
                  >
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-slate-300 flex items-center gap-2">
                    <Camera className="w-4 h-4" /> 구도
                  </label>
                  <select 
                    value={config.composition}
                    onChange={(e) => setConfig(prev => ({ ...prev, composition: e.target.value as any }))}
                    className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-indigo-500 focus:outline-none appearance-none text-sm"
                  >
                    <option value="front">정면</option>
                    <option value="side">측면</option>
                    <option value="back">후면</option>
                    <option value="three-quarter">3/4 각도</option>
                    <option value="dynamic">역동적 각도</option>
                  </select>
                </div>
              </div>
          ...
          <div className="bg-slate-900 rounded-2xl p-6 border border-slate-800">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider">생성된 프롬프트</h3>
              <button 
                onClick={copyToClipboard}
                className="text-xs flex items-center gap-1 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg transition-colors"
              >
                <Copy className="w-3 h-3" /> 복사
              </button>
            </div>
            <div className="bg-black/30 rounded-lg p-4 font-mono text-xs text-slate-300 leading-relaxed border border-slate-800/50 max-h-64 overflow-y-auto">
